...
description: "Ship products directly to an address with the Direct Mail send method."
---

When you already know the address you want to send an order to, you can use Goody to ship directly to that address using the Direct Mail send method (`direct_mail`).

There are two things to be aware of when using direct mail:
...
When sending a gift with Goody, you don't need to specify the variants since the recipient can select the variants on their own. However, when sending with Direct Mail, we need to know which variant to send to the recipient, so you'll need to specify the variant.
...
## Example direct mail payload

This payload will send a direct mail gift to the specified recipient. This product is a pack of cookies with a single variant selection (`variants_num_selectable` is `1`).

```json
{
  "from_name": "Michael",
  "send_method": "direct_mail",
  "recipients": [
    {
      "first_name": "Alena",
      "last_name": "Kenter",
