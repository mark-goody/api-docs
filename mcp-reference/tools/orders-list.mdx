...
<ResponseField name="orders" type="object[]">
  ...
  <Expandable title="order">
    ...
    <ResponseField name="funding_source" type="string | null">`card`, `balance`, `workspace_balance`, `corporate_account`, or `commerce_stored_value`; null when unknown or for commerce app orders. For an order, a successful legacy repayment uses the card that collected the payment.</ResponseField>
