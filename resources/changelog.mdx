...
- **Payment method visibility.** Order and order-batch responses will include `funding_source` and a nullable `payment_method_id`. Cards use a Goody UUID; account-funded payments use a null ID and a separate funding source. Order reads reflect the collected card after a successful legacy repayment.
