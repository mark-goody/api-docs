...
## Example: tracking numbers enabled

`tracking_number` and `tracking_url` are only populated for approved distribution partners. When they are enabled on your app, both fields are filled in on every shipment in the payload. Nothing else about the payload changes, and no fields are added or removed — the two fields are always present, and enabling them changes their value from `null` to the shipment's tracking details.
...
<Note>
  `tracking_url` is the carrier's public tracking page for the shipment. It can
  still be `null` when the carrier does not provide a tracking page, even with
  tracking numbers enabled, so treat it as optional rather than assuming a
  `tracking_number` always comes with a URL.
</Note>
...
<ResponseField name="data" type="object">
  ...
  <ResponseField name="shipments" type="array[object]" required>
    ...
    <Expandable title="properties">
      ...
      The carrier's public tracking page for this shipment. Only for approved distribution partners.
      </ResponseField>
