...
<ResponseField name="payment_method_id" type="string | null">The funding source used for the batch. Card payments return the Goody payment method UUID; account-funded payments return an identifier such as `BALANCE`, `WORKSPACE_BALANCE`, `COMMERCE_STORED_VALUE`, or `CORPORATE_ACCOUNT`.</ResponseField>
