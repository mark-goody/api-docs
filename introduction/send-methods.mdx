...
The send method (`email_and_link`, `link_multiple_custom_list`, or `direct_mail`) is passed as the `send_method` field on the [create order batch payload](/api-reference/order-batches/create-an-order-batch).
...
Ship products directly to the given address. For more info, see the [documentation page for direct mail](/introduction/send-to-address). The address must be specified in the `mailing_address` field of the recipient payload, like the example below:
