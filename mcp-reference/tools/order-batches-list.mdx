...
<ResponseField name="order_batches" type="object[]">
  ...
  <Expandable title="order batch">
    ...
    <ResponseField name="funding_source" type="string | null">`card`, `balance`, `workspace_balance`, `corporate_account`, or `commerce_stored_value`; null when unknown. For a batch, this describes the selected funding source, not a combined payment result for its orders.</ResponseField>
    <ResponseField name="payment_method_id" type="string (uuid) | null">Goody card payment method UUID, including inactive cards. Null for account funding or when the card is unknown.</ResponseField>
