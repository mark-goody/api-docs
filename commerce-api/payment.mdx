...
It relies on a stored value balance deposited with Goody. Revenue share goes back to the same account, so no additional revenue share reconciliation is required.

1. Create an order batch with `payment_method_id` set to the ID given to you by your partnerships manager.
